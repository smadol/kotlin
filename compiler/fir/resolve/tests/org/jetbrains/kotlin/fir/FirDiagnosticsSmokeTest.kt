...
abstract class AbstractFirDiagnosticsSmokeTest : BaseDiagnosticsTest() {
    ...
    fun analyzeAndCheckUnhandled(testDataFile: File, files: List<TestFile>) {
        ...
        val configToSession = modules.mapValues { (config, info) ->
            val moduleFiles = groupedByModule[config]!!
        ...
        for ((testModule, testFilesInModule) in groupedByModule) {
            ...
            val session = configToSession[testModule]!!
    ...
    val builtInsModuleInfo = BuiltInModuleInfo(Name.special("<built-ins>"))

//    protected open fun getAdditionalDependencies(module: TestModule): List<ModuleInfo> =
//        emptyList()
    ...
    protected open class TestModuleInfo(override val name: Name) : ModuleInfo {
        override val platform: TargetPlatform?
            get() = JvmPlatform
