...
class FirAccessResolveTransformer : FirAbstractTreeTransformerWithSuperTypes(reversedScopePriority = true) {
    ...
    override fun transformAccessExpression(accessExpression: FirAccessExpression, data: Nothing?): CompositeTransformResult<FirStatement> {
        return withNewSettings {
            lookupProperties = true
            lookupFunctions = false
            super.transformAccessExpression(accessExpression, data)
        }
    }
