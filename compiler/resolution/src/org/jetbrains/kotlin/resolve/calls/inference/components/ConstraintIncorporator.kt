...
//                )
        ...
        if (baseConstraint.kind != ConstraintKind.UPPER) {
            c.addNewIncorporatedConstraint(approximateCapturedTypes(typeForApproximation, toSuper = false), targetVariable.defaultType(c))
        }
        if (baseConstraint.kind != ConstraintKind.LOWER) {
            c.addNewIncorporatedConstraint(targetVariable.defaultType(c), approximateCapturedTypes(typeForApproximation, toSuper = true))
        }
    }
